...
public class SubscriptionClientExtensionsTests(
    AzureCloudServiceBusResource<TestExistingNamespaceAzureServiceBusOptions> serviceBusResource)
    : IClassFixture<AzureCloudServiceBusResource<TestExistingNamespaceAzureServiceBusOptions>>
{
    ...
    public async Task ReceivedMessage_NoTask_DeserializeContent_CreateDto()
    {
        ...
        ITopicClient topicClient = serviceBusResource.GetTopicClient("foo");
        ISubscriptionClient subscriptionClient = serviceBusResource.GetSubscriptionClient("foo", "test1");
